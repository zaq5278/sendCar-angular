...
.config(function($stateProvider) {
    ...
    $scope.login = function(userName,passWord) {
        ...
        var loginUrl = myUrl+"synear/userServiceController.do?applogin&userName=" + userName + "&passWord=" + passWord + "&callback=JSON_CALLBACK";
        $http.jsonp(loginUrl).success(function (result) {
            $scope.lodingHide();
            if(result.msgCode == "0001"){
                sessionStorage.functionArray = JSON.stringify(result.function);
    ...
    $scope.checkTheOrder = function(urlType) {
        ...
        if(urlType == "checkOrder"){
            url= myUrl+"synear/sendmanagerServiceController.do?getSapSaleOrderList&sessionid=" + sessionStorage.sessionid + "&ordercode=" + orderNumber + "&createDate=" + beforeTime + "&ordertime=" + lastTime + "&callback=JSON_CALLBACK";
        }
        if (urlType == "savedOrder"){
            url = myUrl+"synear/sendmanagerServiceController.do?datagrid&sessionid=" + sessionStorage.sessionid + "&fstatus=N" + "&sendid=" + orderNumber + "&billingdate_begin=" + beforeTime + "&billingdate_end=" + lastTime + "&callback=JSON_CALLBACK";
        }
        if (urlType == "submitOrder"){
            url = myUrl+"synear/sendmanagerServiceController.do?datagrid&sessionid=" + sessionStorage.sessionid + "&fstatus=Y" + "&sendid=" + orderNumber + "&billingdate_begin=" + beforeTime + "&billingdate_end=" + lastTime + "&callback=JSON_CALLBACK";
        }
    ...
    $scope.goToRevisrTheHavedOrder = function(index){
        $scope.lodingShow("加载中...");
        var url = myUrl+"synear/sendmanagerServiceController.do?mxdatagrid"+ "&sessionid=" + sessionStorage.sessionid + "&fstatus=N" + "&fid=" + $scope.theCheckOrderList[index].id + "&callback=JSON_CALLBACK";
        $http.jsonp(url).success(function(result){
            if(result.msgDesc == "需要重新登录！"){
                $scope.showAlert(result.msgDesc);$state.go("login");
            }else {
    ...
    $scope.delTheSavedOrder = function(index){
        ...
        var url = myUrl+"synear/sendmanagerServiceController.do?doDel" + "&sessionid=" + sessionStorage.sessionid + "&id=" + $scope.theCheckOrderList[index].id + "&callback=JSON_CALLBACK";
        $http.jsonp(url).success(function(result){
            $scope.lodingHide();
            if(result.msgDesc == "需要重新登录！"){
                $scope.showAlert(result.msgDesc);$state.go("login");
    ...
    $scope.submitTheSavedOrder = function(index){
        ...
        var url = myUrl+"synear/sendmanagerServiceController.do?doUpdate" + "&sessionid=" + sessionStorage.sessionid  + "&freightstatus=" + "&id=" + theOrder.id + "&drivernumber=" + theOrder.drivernumber + "&sjusername=" + theOrder.sjusername + "&phone=" + theOrder.phone + "&ordertime=" + newTime + "&isheche=" + theOrder.isheche + "&drivertype=" + theOrder.drivertype +"&callback=JSON_CALLBACK";
    ...
    $scope.updateTheOrder = function(){
        ...
        var url = myUrl+"synear/sendmanagerServiceController.do?doUpdate" + "&sessionid=" + sessionStorage.sessionid  + "&freightstatus=" + "&id=" + sessionStorage.id + "&drivernumber=" + carNumberInput + "&sjusername=" + driverInput + "&phone=" + phoneInput + "&ordertime=" + newTime + "&isheche=" + isHeChe + "&drivertype=" + carType +"&callback=JSON_CALLBACK";
        $http.jsonp(url).success(function(result){
            if (result.msgDesc == "需要重新登录！"){
                $scope.lodingHide();
                $scope.showAlert(result.msgDesc);$state.go("login");
