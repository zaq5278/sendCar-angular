...
angular.module('myApp.sendCarController', [])
    ...
    $scope.updateTheOrder = function(){
        ...
        var url = myUrl+"synear/sendmanagerServiceController.do?doUpdate" + "&sessionid=" + sessionStorage.sessionid  + "&freightstatus=" + "&id=" + sessionStorage.id + "&drivernumber=" + carNumberInput + "&sjusername=" + driverInput + "&phone=" + phoneInput + "&ordertime=" + time + "&isheche=" + isHeChe + "&drivertype=" + carType +"&callback=JSON_CALLBACK";
        $http.jsonp(url).success(function(result){
            if (result.msgDesc == "需要重新登录！"){
                $scope.lodingHide();
                $scope.showAlert(result.msgDesc);$state.go("login");
